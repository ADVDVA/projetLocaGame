...
public class EntityFinderImpl<T> implements EntityFinder<T>, Serializable {
	...
	// requête dynamique pour  recherche
	@Override
	...
	private <K, V> void setParameters(Query query, Map<K, V> param) {
		...
		while(itr.hasNext()){
			...
			if((boolean) entry.getKey().toString().toLowerCase().contains("date")) // pas obliger de placer le mot date dans le champ du type date
